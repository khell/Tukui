...
RaidUtilityPanel:CreatePanel("Default", T.Scale(170), panel_height, "TOP", UIParent, "TOP", -324, panel_height + 15)
...
local function CreateButton(name, parent, template, width, height, point, relativeto, point2, xOfs, yOfs, text, texture)
	...
	if text then
		...
		t:SetFont(C.media.font, C["datatext"].fontsize)
		t:SetPoint("CENTER")
...
CreateButton("ShowButton", RaidUtilityPanel, "UIMenuButtonStretchTemplate, SecureHandlerClickTemplate", RaidUtilityPanel:GetWidth() / 2.5, T.Scale(18), "TOP", UIParent, "TOP", -324, 2, "Raid Utility", nil)
ShowButton:SetAttribute("_onclick", [=[
 if select(5, self:GetPoint()) > 0 then
	 self:GetParent():ClearAllPoints()
	 self:GetParent():SetPoint("TOP", UIParent, "TOP", -324, 1)
	 self:ClearAllPoints()
	 self:SetPoint("TOP", UIParent, "TOP", -324, -100)
 else
	 self:GetParent():ClearAllPoints()
	 self:GetParent():SetPoint("TOP", UIParent, "TOP", -324, 500)
	 self:ClearAllPoints()
	 self:SetPoint("TOP", UIParent, "TOP", -324, 1) 
 end
]=])
